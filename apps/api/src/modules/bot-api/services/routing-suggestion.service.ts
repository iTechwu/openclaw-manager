...
export class RoutingSuggestionService {
  ...
  async generateSuggestions(
    ...
    this.logger.info('Generating routing suggestions', {
      ...
      totalModels: providers.reduce(
        (sum, p) => sum + p.allowedModels.length,
        0,
      ),
    });
    ...
    const functionRouteRules =
      this.generateFunctionRouteRules(modelCapabilities);
    ...
    const failoverSuggestion =
      this.generateFailoverSuggestion(modelCapabilities);
  ...
  private generateSummary(
    ...
    const topModels = capabilities
      .map((cap) => {
        const avgScore =
          Object.values(cap.score).reduce((sum, s) => sum + s, 0) /
          Object.values(cap.score).length;
