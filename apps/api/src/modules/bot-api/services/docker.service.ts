    ...
    const container = await this.docker.createContainer({
      ...
      Cmd: [
        ...
        if [ "$PROVIDER" = "custom" ] && [ -n "$AI_API_TYPE" ]; then
          ...
          # When using a proxy (PROXY_URL is set), use openai-compatible as the model provider
          ...
          # When using a proxy with standard providers, also use openai-compatible
          if [ -n "$PROXY_URL" ]; then
            MODEL_PROVIDER="openai-compatible"
          else
...
YAML_EOF
          ...
          echo "Not using openai-compatible provider, skipping config.yaml creation"
        fi
